...
from models import generate_practice_sentences, generate_random_sentence_with_ngram
import time
...
if st.button("Analyze Typing"):
    ...
    # Generate a new reference sentence with problematic n-grams
    if problematic_sequences:
        new_sentence = " ".join([generate_random_sentence_with_ngram(ngram) for _, ngram in problematic_sequences.keys()])
        print(f'{new_sentence = }')
        st.session_state.reference_sentence = new_sentence
